...
exports.views = function(req, res,callback){
	async.auto({
		...
		getUser: function get_user(callback){
			...
			}) //Function returns information for all users.  Similar to 'select * from users;'
				.then(function(getUser){
					callback(null, getUser); //On a successful query the results are returned in the object 'getUser'
				})
				.catch(function(err){
	...
	Users.all()
		.then(function(getUsers){
			...
			for (k=0; k<getUsers.length; k++){
				if (getUsers[k].email === req.user.email){
					exists = true;
					Users.update({loggedIn : 1}, ({where: ["email=?", req.user.email]}))
					.then(function(){})
					.catch(function(err){
...
exports.logout = function(req, res, callback){
	if (req.user === 'undefined'){
		...
		Users.update({loggedIn : 0}, ({where: ["email=?", req.user.email]}))
		.then(function(){
